...
from data.dataset import Gowalla, Foursquare, Amazon, GowallaICS, FoursquareICS, AmazonICS
...
if __name__ == "__main__":
    ...
    logfile = logging.FileHandler(os.path.join(work_root+'exp_cache/',args.dataset+"-"+args.pretrain_model,str(curtime)+"_item_coldstart.txt"))
    logfile.setFormatter(fmt)
    logger.addHandler(logfile)

    if args.dataset == 'gowalla':
        data = GowallaICS(data_root=work_root+'data/')
    elif args.dataset == 'foursquare':
        data = FoursquareICS(data_root=work_root+'data/')
    elif args.dataset == 'amazon':
        data = AmazonICS(data_root=work_root+'data/')
    else:
    ...
    # X_item = torch.sparse_coo_tensor(torch.arange(dim_features_item).unsqueeze(0).repeat(2,1),
    #                                  torch.ones(dim_features_item),torch.Size([dim_features_item,dim_features_item])).to(device)
    ...
    # net_ds = GCFSignal(dim_emb).to(device)
    ...
    for epoch in range(1,epochs+1):
        ...
        if ret['recall'][0] > best_val:
            ...
            # user_best_state = deepcopy(net.state_dict())
            # item_best_state = deepcopy(net_item.state_dict())
