...
def test_united(embs, testRatings, testNegatives):
    ...
    with torch.no_grad():
        for u in user:
            ...
            re = test_one_user(u, test_item, neg_item, embs)
            result['recall'] += re['recall'] / n_test_users
            result['ndcg'] += re['ndcg'] / n_test_users
    return result


def test_one_user(user,test_item,neg_item,embs):
    u = user
    user_pos_test = test_item
    test_items = user_pos_test+neg_item
    users = np.full(len(test_items), u)
    predictions = torch.sum(embs[users] * embs[test_items], dim=1).cpu().tolist()
    rating = dict(zip(test_items, predictions))
